...
namespace ToolKit
{
  ...
  Entity* Sphere::CopyTo(Entity* copyTo) const { return Super::CopyTo(copyTo); }
  ...
  Entity* Cone::CopyTo(Entity* copyTo) const { return Super::CopyTo(copyTo); }
  ...
  void Arrow2d::Generate(AxisLabel axis)
  {
    ...
    {
      newMat->SetColorVal(Vec3(0.537f, 0.831f, 0.07f));
      rotation = glm::angleAxis(glm::half_pi<float>(), Z_AXIS);
    }

    if (m_label == AxisLabel::Z)
    {
      newMat->SetColorVal(Vec3(0.196f, 0.541f, 0.905f));
  ...
  Entity* LineBatch::CopyTo(Entity* copyTo) const { return Super::CopyTo(copyTo); }
