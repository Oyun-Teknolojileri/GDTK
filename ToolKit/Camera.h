...
namespace ToolKit
{
  ...
  /**
   * Represents camera data in gpu.
   * Updated with Renderer::SetCamera.
   */

  struct TK_API CameraCacheItem : CacheItem
  {
    struct Data
    {
      Vec3 position;
  ...
  class TK_API Camera : public Entity, public ICacheable
  {
   public:
    ...
    /** Invalidates memory caches items along with spatial caches. */
    void InvalidateSpatialCaches() override;

    /** Returns up to date cache item. */
    const CameraCacheItem& GetCacheItem() override;
   ...
   private:
    ...
    /** Used to apply zoom by adjusting camera frustum. */
    float m_orthographicScale = 0.001f;

    Mat4 m_projection;

    CameraCacheItem m_cacheItem;
  };
