...
namespace ToolKit
{
  ...
  void Camera::InvalidateSpatialCaches()
  {
    Super::InvalidateSpatialCaches();
    InvalidateCacheItem();
  }

  const CameraCacheItem& Camera::GetCacheItem()
  {
    if (m_cacheItem.isValid)
    {
      return m_cacheItem;
    }
    ...
    m_cacheItem.data.farPlane                  = Far();
    m_cacheItem.data.direction                 = Direction();
    m_cacheItem.data.projection                = GetProjectionMatrix();
    m_cacheItem.data.view                      = GetViewMatrix();
    m_cacheItem.data.projectionView            = GetProjectViewMatrix();

    // Calculate projection view matrix without translation.
    Mat4 viewNoTr                              = m_cacheItem.data.view;
    viewNoTr[0][3]                             = 0.0f;
    viewNoTr[1][3]                             = 0.0f;
    viewNoTr[2][3]                             = 0.0f;
    viewNoTr[3][3]                             = 1.0f;
    ...
    m_cacheItem.data.projectionViewNoTranslate = m_cacheItem.data.projection * viewNoTr;
    m_cacheItem.Validate();

    return m_cacheItem;
  }

  void Camera::InvalidateCacheItem() { m_cacheItem.Invalidate(); }
  ...
  void Camera::ParameterEventConstructor()
  {
    ...
    auto updateLensInternalFn = [this]()
    {
      if (m_ortographic)
      {
        SetLens(Left(), Right(), Bottom(), Top(), Near(), Far());
      }
      else
      {
