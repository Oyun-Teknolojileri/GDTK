...
namespace ToolKit
{
  ...
  void ReadAttr(XmlNode* node, const String& name, ObjectId& val) { val = ReadVal<ObjectId>(node, name, val); }
  ...
  String ToAbsolutePath(const String& path)
  {
    ...
    String absolutePath     = PathToString(p);
  ...
  String GetRelativeResourcePath(const String& path, String* rootFolder)
  {
    ...
    if (root.empty())
    {
      // No root is set, test engine dir.
      exist = String::npos;
    }
