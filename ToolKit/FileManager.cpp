...
namespace ToolKit
{
  ...
  void FileManager::CreateResourceFolder(const String& folder)
  {
    String root = ResourcePath();
    if (!StartsWith(folder, root))
    {
      TK_ERR("Folder must be under resource path.");
      return;
    }
    ...
    if (err)
    {
      TK_ERR("Failed to create directory: %s", err.message().c_str());
    }
  }
    ...
    if (offset != 0)
    {
      if (unzSetOffset64(zfile, offset) == UNZ_OK)
      {
        if (unzOpenCurrentFile(zfile) == UNZ_OK)
        {
          ...
          {
            // Read file
            bufferSize        = (uint) unzFileInfo.uncompressed_size;
      ...
      do
      {
        if (unzOpenCurrentFile(m_zfile) == UNZ_OK)
        {
          ...
          if (unzGetCurrentFileInfo(m_zfile, &fileInfo, NULL, 0, NULL, 0, NULL, 0) == UNZ_OK)
          {
            ...
            element.second = (uint) fileInfo.uncompressed_size;
