...
> Living log - updated each phase. Last updated: 2026-08-03 (Phase 2a step 4 done).

**Current status:** Phase 2a in progress — step 4 of 7 done. Next: step 5 (Renderer write/bind + ForwardPass variant selection).

| Phase | Status |
|---|---|
| 0 - Doc sync + dead-code cleanup | done (2026-07-25) |
| 1 - Instrumentation | **done (2026-07-26)** |
| 2 - Instance data transport (2a/2b) | **2a step 4 done (2026-08-03)** |
...
| **MOD** `Renderer.h` | `using InstanceRecord2a = PerDrawUboLayout;` (next to line ~184) + `<cstddef>` for `offsetof`. static_asserts (DONE step 1): `sizeof % 16 == 0`, `offsetof(model) == 0`, `sizeof == 70*16`; `static constexpr uint InstanceRecord2aStride = sizeof/16`. The `alignof >= 16` guard is NOT asserted here — see Changelog v10 (glm's 4-byte alignment + 2a texture transport make it a 2b concern on the authored lean `InstanceRecord`). Private members (step 5): `InstanceDataBuffer m_instanceBuffer;`, `bool m_instancedTransportEnabled = false;` + public `IsInstancedTransportEnabled()`. |
